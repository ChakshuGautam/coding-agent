...
var GitAddFileDefination = &genai.FunctionDeclaration{
	Name:        "gitAddFile",
...
func GitAddFile(input *genai.FunctionCall) (string, error) {
	if err := EnsureGitignore(); err != nil {
		return "", fmt.Errorf("could not verify .gitignore: %v", err)
	}

	var name string
	if val, ok := input.Args["name"]; ok && val != nil {
		if strVal, ok := val.(string); ok {
			name = strings.TrimSpace(strVal)
		}
	}
	...
	if name == ".env" {
		return "", fmt.Errorf(".env file is explicitly blocked from staging")
	}
	...
	if err != nil {
		return "", fmt.Errorf("error running git add: %v\n%s", err, output)
	}

	// Check if .env is in staged files
	checkCmd := exec.Command("git", "diff", "--cached", "--name-only")
	stagedFiles, err := checkCmd.Output()
	if err == nil && strings.Contains(string(stagedFiles), ".env") {
		// Attempt to unstage .env; ignore error if it fails
		_ = exec.Command("git", "reset", "--quiet", ".env").Run()
		return string(output) + "\n.env was removed from staging area for safety.", nil
	}
